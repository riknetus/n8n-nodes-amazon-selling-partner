...
async function createDataKioskQuery(accessToken) {
    console.log('📊 Creating Data Kiosk Query with CORRECT Structure...\n');
    
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - 7); // Last 7 days for faster testing
    const endDate = new Date();
    const startDateStr = startDate.toISOString().split('T')[0];
    const endDateStr = endDate.toISOString().split('T')[0];
    
    // CORRECT structure: parameters go on salesAndTrafficByDate, not on parent
    const graphqlQuery = `
        query {
...
async function main() {
    ...
    console.log('Testing DataKiosk API - CORRECT STRUCTURE');
    console.log('═'.repeat(70));
    console.log();
    ...
    try {
        ...
        console.log('\n📝 Key Finding:');
        console.log('   Parameters go on salesAndTrafficByDate, NOT on parent field!');
        console.log('\n🔗 Reference:');

...
async function createDataKioskQuery(accessToken) {
    console.log('📊 Creating Data Kiosk Query with Schema Explorer Structure...\n');
    ...
    // Using the EXACT structure from the schema explorer
    const graphqlQuery = `
        query {
            analytics_salesAndTraffic_2024_04_24 {
                salesAndTrafficTrends(
                    ...
                    dateAggregation: DAY
                    endDate: "${endDateStr}"
                    filters: { asins: [], marketplaceId: "${config.marketplaceId}" }
...
async function pollQueryStatus(accessToken, queryId) {
    ...
    const pollInterval = 5000; // 5 seconds (longer for DataKiosk queries)
...
async function downloadDocument(documentUrl) {
    ...
    try {
        ...
        console.log('\nData Preview (first 1500 chars):');
        const dataStr = JSON.stringify(response.data, null, 2);
        console.log(dataStr.substring(0, 1500));
...
async function main() {
    ...
    console.log('Testing DataKiosk API - FINAL CORRECT STRUCTURE');
    console.log('Using Schema Explorer Query Format');
    console.log('═'.repeat(70));
    console.log();
    ...
    try {
        ...
        console.log('═'.repeat(70));
        console.log('\n✅ DataKiosk API is FULLY WORKING!');
        console.log('✅ Credentials are valid and have Brand Analytics access');
        console.log('✅ Query structure is correct');
        console.log('\n📝 Key Findings:');
        console.log('   • Versioned field: analytics_salesAndTraffic_2024_04_24');
        console.log('   • Query field: salesAndTrafficTrends (not salesAndTrafficByDate)');
        console.log('   • dateAggregation: DAY (not aggregateBy: DATE)');
        console.log('   • Filters use object syntax: { marketplaceId: "..." }');
        console.log('\n🔗 Reference:');
        console.log('   https://sellercentral.amazon.com/datakiosk-schema-explorer?schema=analytics_salesAndTraffic_2024_04_24\n');
